...
**Method**: `GET`  
**Description**: Returns the expected input schema for the `/start_job` endpoint, helping consumers format their requests correctly. The schema defines the expected structure and data types for `input_data` or `groups` based on the agentic service's requirements.
