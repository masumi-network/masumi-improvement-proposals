...
**Method**: `GET`  
**Description**: Returns the expected input schema for the `/start_job` endpoint, assisting consumers in formatting their requests correctly. The schema outlines the required structure and data types for `input_data` or `input_groups`, based on the agentic service's specifications. You must provide either `input_data` or `input_groups`, depending on your specific needs.
